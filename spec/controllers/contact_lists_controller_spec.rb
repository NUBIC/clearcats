  ...
  context "logged in as administrator" do
    ...
    describe "GET new" do
      it "assigns a new contact_list as @contact_list" do
        ContactList.stub(:new).and_return(mock_contact_list(:contacts => []))
        get :new
    ...
    describe "POST create" do
      ...
      describe "with valid params" do
        it "assigns a newly created contact_list as @contact_list" do
          ContactList.stub(:new).with({'these' => 'params'}).and_return(mock_contact_list(:save => true, :contacts => []))
          post :create, :contact_list => {:these => 'params'}
          assigns[:contact_list].should equal(mock_contact_list)
        end
    
        it "redirects to the created contact_list" do
          ContactList.stub(:new).and_return(mock_contact_list(:save => true, :contacts => []))
      ...
      describe "with invalid params" do
        it "assigns a newly created but unsaved contact_list as @contact_list" do
          ContactList.stub(:new).with({'these' => 'params'}).and_return(mock_contact_list(:save => false, :contacts => []))
          post :create, :contact_list => {:these => 'params'}
          assigns[:contact_list].should equal(mock_contact_list)
        end
    
        it "re-renders the 'new' template" do
          ContactList.stub(:new).and_return(mock_contact_list(:save => false, :contacts => []))
          post :create, :contact_list => {}
          response.should render_template('new')
        end
        
        it "re-renders the new template if a contact_list already exists with that name" do
          name = "asdf"
          Factory(:contact_list, :name => name)
          post :create, :contact_list => { :name => name }
          response.should render_template('new')
        end
    ...
    describe "GET edit" do
      it "assigns the requested contact_list as @contact_list" do
        ContactList.stub(:find).with("37").and_return(mock_contact_list(:contacts => []))
        get :edit, :id => "37"
        assigns[:contact_list].should equal(mock_contact_list)
      end
    end
    ...
    describe "PUT update" do
      ...
      describe "with valid params" do
        it "updates the requested contact_list" do
          ContactList.should_receive(:find).with("37").and_return(mock_contact_list(:contacts => []))
          mock_contact_list.should_receive(:update_attributes).with({'these' => 'params'})
          put :update, :id => "37", :contact_list => {:these => 'params'}
        end
    
        it "assigns the requested contact_list as @contact_list" do
          ContactList.stub(:find).and_return(mock_contact_list(:update_attributes => true, :contacts => []))
          put :update, :id => "1"
          assigns[:contact_list].should equal(mock_contact_list)
        end
      ...
      describe "with invalid params" do
        ...
        it "assigns the contact_list as @contact_list" do
          ContactList.stub(:find).and_return(mock_contact_list(:update_attributes => false, :contacts => []))
          put :update, :id => "1"
          assigns[:contact_list].should equal(mock_contact_list)
        end
